...
try {
  ...
  // Fallback to empty configuration if file doesn't exist or is invalid
}
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (cleanUrl.endsWith('.mp4')) {
          const filePath = path.join(__dirname, 'public', decodeURIComponent(cleanUrl))
          if (fs.existsSync(filePath)) {
            ...
            if (range) {
              ...
              const end = Math.min(isNaN(rawEnd) ? stat.size - 1 : rawEnd, stat.size - 1)
              const start = Math.min(isNaN(rawStart) ? 0 : rawStart, end)
        ...
        if (!robotsTxt || cleanUrl !== '/robots.txt') return next()
